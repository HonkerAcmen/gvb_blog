...
func (ImageFileAPI) ImageFilesUpload(c *gin.Context) {
	form, err := c.MultipartForm()
	if err != nil {
		res.FailWithMessage(err.Error(), c)
		return
	}
	FileList, ok := form.File[string(filesapi.ImageFile)]
	if !ok {
		if err != nil {
	...
	for _, file := range FileList {
		filePath := path.Join("uploads/" + string(filesapi.ImageFile) + "/" + file.Filename)
		var resLine []filesapi.UploadFileInfo
		fileType := filesapi.FilesAPI.GetFileType(filesapi.FilesAPI{}, filePath)
		if fileType != filesapi.ImageFile {
			res.FailWithMessage("文件类型错误", c)
			continue
		}
		fileSize, err := filesapi.FilesAPI.GetFileSize(filesapi.FilesAPI{}, fileType, int64(global.Config.UploadInfo.PhotoSize))
		if err != nil {
			res.FailWithMessage(err.Error(), c)
			continue
		}
